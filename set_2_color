...
# canvas.center()
canvas.align(lv.ALIGN.TOP_LEFT, 5, 5)
...
cw.set_size(80, 80)
cw.align(lv.ALIGN.TOP_RIGHT, -20, 20)

def btn_event_cb(evt):
    color = cw.get_rgb()
    print(hex(color.full))
    print(evt.code)
    hds2xx_lv_colors[1] = color
    display()


cw.add_event_cb(btn_event_cb, lv.EVENT.PRESSING, None) 
# color = cw.get_rgb()
# print(hex(color.full))
